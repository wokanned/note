...
| **电源完整性 PI**  | 去耦电容布局         | 芯片瞬态电流供应不足   | 紧贴管脚，容值梯级（100nF → 10nF → 1nF）        | 二       |
| **电源完整性 PI**  | DC 压降（IR Drop）   | 电源线过细导致压降过大 | 1mm 铜皮约通 1~2A，大电流需加宽加厚               | 三       |
...
| **电磁兼容 EMC**   | 板边缝合孔           | 电磁从板边泄露辐射     | 板边一圈密集 GND 过孔，间距 < λ/20               | 三       |
...
| **叠层/过孔**      | 背钻（Back-drill）   | 过孔残桩引发谐振       | 25Gbps+ 高速 SERDES 必须背钻                      | 四       |
| **叠层/过孔**      | 散热过孔阵列         | 大功率器件散热不足     | 芯片底部矩阵打过孔，铜填充导热                    | 三       |
| **布局布线**       | 模块化分区布局       | 模块间互相干扰         | 电源/数字/模拟/高速接口各自独立分区               | 三       |
| **布局布线**       | 禁止 90° 直角       | 走线拐角阻抗突变       | 一律改用 45° 斜角或圆弧                          | 四       |
...
孔径需要小于15mil（8~12mil）防止热盘漏锡导致热盘虚焊。四层板（7.874/19.874）
...
- 0欧姆电阻——并非0欧姆（50mR左右）。作用：在高频信号下充当电感电容解决emc问题，或者用作功率地和信号地之间的单点接地（→见三章「数字地/模拟地间距」：0Ω 是地分割/单点接地的实施手段）
...
- 在1oz铜厚下跑1A电流推荐20~30mil（0.5mm）左右，2A-3A则是31.5-47.2mil（0.8-1.2mm），若是更高优先铺铜并选更厚的铺铜（→见知识地图 PI「DC 压降」行，可互相换算验证）
...
- 嘉立创打盲埋孔需要选特殊板材
- 晶振下方不能走信号线，元件，（不能铺铜（四层注意GND和PWR层）--存疑），不要靠近板边。！靠近mcu放置，（→见四章「缝合地过孔」：晶振旁的地孔屏蔽与板边缝合孔同属接地屏蔽手法）
...
- 最大元凶：过孔残桩（Stub）：如果信号从第 1 层走到第 3 层，过孔在第 4~6 层多余的 "尾巴" 就是残桩，它会引发严重的信号反射和谐振，是高速信号（DDR3/4、千兆网、USB3.0）的头号杀手。通过背钻工艺将多余的孔铜钻掉来解决这种影响（→见知识地图「叠层/过孔」背钻行）
